...
void loop()
{
  ...
  if (millis() - lastTime >= 5000) { // Update every one second
    ...
    distance = distance + (pulseCount * 13.5 / 100 / 1000); // Calculate distance
...
int printDisplay() {
    // The format for the output to be proper JSON should be: {"rpm": 100, "distance": 500, "velocity": 30}.
    Serial.print("{\"rpm\": ");
    Serial.print(rpm);
